...
DEFAULT_SOURCE_URL = 'http://biometrics.nist.gov/cs_links/EMNIST/gzip.zip'
...
def extract_images(f):
  ...
  with gzip.GzipFile(fileobj=f) as bytestream:
    ...
    if magic != 2051:
      raise ValueError('Invalid magic number %d in EMNIST image file: %s' %
                       (magic, f.name))
...
def extract_labels(f, one_hot=False, num_classes=10):
  ...
  with gzip.GzipFile(fileobj=f) as bytestream:
    magic = _read32(bytestream)
    if magic != 2049:
      raise ValueError('Invalid magic number %d in EMNIST label file: %s' %
                       (magic, f.name))
